...
class Transformer(nn.Module):
    def __init__(self, args, activation="relu"):
        ...
        if self.multiscale:
            ...
            self.encoder = DeformableTransformerEncoder(
                args, encoder_layer, self.enc_layers
            )
    ...
    def forward(self, srcs, masks, query_embed, pos_embeds):
        if self.multiscale:
            ...
            return self.forward_single_scale(
                srcs[0], masks[0], query_embed, pos_embeds[0]
            )
    ...
    def forward_single_scale(self, src, mask, query_embed, pos):
        ...
        if self.args.smca:
            grid_y, grid_x = torch.meshgrid(
                torch.arange(0, memory_h), torch.arange(0, memory_w)
            )
    ...
    def forward_multi_scale(self, srcs, masks, query_embed, pos_embeds):
        ...
        orig_pos_embed_16 = pos_embeds[0] + self.level_embed[0].unsqueeze(0).unsqueeze(
            -1
        ).unsqueeze(-1)
        ...
        orig_pos_embed_32 = pos_embeds[1] + self.level_embed[1].unsqueeze(0).unsqueeze(
            -1
        ).unsqueeze(-1)
        ...
        orig_pos_embed_64 = pos_embeds[2] + self.level_embed[2].unsqueeze(0).unsqueeze(
            -1
        ).unsqueeze(-1)
        ...
        grid_y_16, grid_x_16 = torch.meshgrid(
            torch.arange(0, h_16), torch.arange(0, w_16)
        )
        ...
        grid_y_32, grid_x_32 = torch.meshgrid(
            torch.arange(0, h_32), torch.arange(0, w_32)
        )
        ...
        grid_y_64, grid_x_64 = torch.meshgrid(
            torch.arange(0, h_64), torch.arange(0, w_64)
        )
